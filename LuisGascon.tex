...
\resumeItemListStart
\resumeItem{Collaborated in the rewrite of legacy RESTful API microservices into a monolith with improved error logging and developer experience}
\resumeItem{Write extensive unit and end-to-end tests using Jest for each API endpoint implemented, ensuring high code coverage and enhanced reliability}
...
\resumeItemListStart
\resumeItem{Developed Excel templates to automate data entry, enhancing efficiency and accuracy through formula-based transformations}
\resumeItem{Documented technical procedures using MkDocs to easily reference solutions for common problems on the go from a website}
\resumeItem{Performed A/V repairs and computer troubleshooting on time to prevent lecture interruption}
\resumeItem{Optimized a Visual Basic app that generates static HTML files for the department's website by skipping over unchanged directories}
...
\small{\item{
              ...
              \vspace{3pt}
              \textbf{Technologies}{: Linux, Git, Heroku, Docker, PostgreSQL, GraphQL, MongoDB, Jest} \\ \vspace{3pt}
